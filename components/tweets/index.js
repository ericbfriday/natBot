...
module.exports = {
        ...
        if (config.settings.monitorStream) {
            client.stream('user', function (stream) {
                ...
                stream.on('error', function (error) {
                    console.log('Error detected with streaming API.'); //jshint ignore: line
                    console.log(error); //jshint ignore: line
                });
            });
        } else if (!config.settings.monitorStream) {
            console.log('\nWarning: Monitoring Twitter stream is currently disabled \ndue to configuration setting.'); //jshint ignore: line
        } else {
            console.log('\nWarning: Something happened while trying watch the Twitter Stream and I\'m not sure what it is.'); //jshint ignore: line
        }
    },
    ...
    countReplies: function (user) {
        ...
        if (countReplies >= 5) {
            console.log('Warning: Replies to @' + user + ' are Temporarily paused so we don\'t spam them.');  //jshint ignore: line
        }
        return countReplies;
    },
    ...
    checkReply: function (tweet) {
        ...
        if (config.settings.randomReplies && x <= config.settings.randomRepliesChance && checkTweet.indexOf(myUsername) === -1 && (_.isNil(tweet.retweeted_status))) {
            ...
            if (replyUsername.toLowerCase() !== config.settings.robotName.toLowerCase() && replyCount < 5) {
                ...
                console.log('\nRandomly replying to the following tweet from @' + replyUsername + ':'); //jshint ignore: line
                console.log(tweet.text);  //jshint ignore: line
        ...
        if (checkTweet.indexOf(myUsername) !== -1 && (_.isNil(tweet.retweeted_status))) {
            ...
            if (replyUsername.toLowerCase() !== config.settings.robotName.toLowerCase() && replyCount < 5) {
                ...
                if (config.personality.otherBots.indexOf(replyUsername.toLowerCase()) !== -1) {
                    console.log('User \'' + replyUsername + '\' is in our bot list. Temporarily limiting replies to this user.'); //jshint ignore: line
                    //config.settings.trackReplies.push(replyUsername,replyUsername,replyUsername,replyUsername,replyUsername,replyUsername);
                }
                //config.settings.trackReplies.push(replyUsername); // Add user to our reply tracker so we don't spam them too much.
                console.log('\nNew reply from @' + replyUsername + ':'); //jshint ignore: line
                console.log(tweet.text); //jshint ignore: line
    ...
    checkInterests: function (tweet) {
        if (tweet.id !== null) {
            ...
            if (foundInterest) {
                ...
                console.log('\n', utils.currentTime(), 'Favoriting the following ' + tweetType + ' from @' + tweetUsername + ' because it mentions \'' + tempInterest + '\':'); //jshint ignore: line
                console.log('\n', tweet.text); //jshint ignore: line
                client.post('favorites/create', {
                    ...
                    if (error) {
                        if (error[0].code === 139) {
                            ...
                            console.log('Error favoriting tweet. Possible API rate limit encountered. Please wait a few moments.'); //jshint ignore: line
                            console.log(error); //jshint ignore: line
                        }
                    }
                });
    ...
    checkStream: function () {
        ...
        if (curTime - config.settings.lastTweetReceivedTime >= 120000) {
            console.log('Stream may have dropped. Restarting stream...'); //jshint ignore: line
            this.watchStream();
        }
    },
    ...
    postNewTweet: function (send_msg) {
        if (config.settings.postTweets) {
            client.post('statuses/update', {
                ...
                if (error) {
                    console.log('Error posting tweet. Possible API rate limit encountered. Please wait a few moments'); //jshint ignore: line
                    console.log(error); //jshint ignore: line
                }
            });
        }
    ...
    writeReply: function (username, replyID, replytext) {
        if (this.checkIgnored(username)) {
            console.log('\nUser is on ignore list. Not replying.'); //jshint ignore: line
        } else {
            ...
            setTimeout(function () { //jshint ignore: line
                ...
                if (typeof myReply !== 'undefined') {
                    console.log('\nGenerating a contextual reply to user.'); //jshint ignore: line
                    replyTweet = '@' + username + ' ' + myReply;
                } else {
                    console.log('\nGenerating a random reply to user.'); //jshint ignore: line
                    replyTweet = generator.twitterFriendly(true, username);
                }
                console.log('\nReplying to user @' + username + ':'); //jshint ignore: line
                console.log(replyTweet); //jshint ignore: line
    ...
    sendReply: function (send_msg, replyID) {
        client.post('statuses/update', {
            ...
            if (error) {
                console.log('Error posting reply. Possible API rate limit encountered. Please wait a few moments'); //jshint ignore: line
                //console.log(error);
            }
        });
    },
    ...
    getFollowers: function () {
        if (!config.settings.followUsers) {
            console.log('\nWarning: Cannot get users list \n\'follow new users\' is disabled in configuration.'); //jshint ignore: line
            return;
        }
        ...
        client.get('followers/list', {
            ...
            if (error) {
                console.log('followers list error', error); //jshint ignore: line
            }
            if (!error) {
                var followerArray = [];
                ...
                client.post('friendships/create', {
                    ...
                    if (error) { console.log('create friendships error', error); }; //jshint ignore: line
                    if (!error) {
                        if (!tweet.following) { // Check response to see whether or not we were already following user. If not, hey! Awesome!
                            console.log('Following user @' + newFollower); //jshint ignore: line
                            //console.log(tweet);
                        }
                    }
                });
